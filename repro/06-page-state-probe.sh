#!/bin/bash
# What a page sees while Chrome holds the window: a local page (probe/) logs visibility, requestAnimationFrame,
# timers and paint timing for 14 s. Arms: headed and headless, each with and without the zero deadline, no WebGPU
# preset; plus two production-config arms that record the --disable-features switches on the browser command line.
...
# Summarise with: node page-state.mjs data/page-state-during-stall.jsonl
set -u
...
trial() {
  ...
  case "$arm" in
    ...
    headed-fix)     mode=--headed; args="$BASE_ARGS,$FIX" ;;
    headless-nofix) mode="";       args="$BASE_ARGS" ;;
    headless-fix)   mode="";       args="$BASE_ARGS,$FIX" ;;
    prod)           mode=--headed; args="$PROD_ARGS"; preset=1 ;;
    prod-nohttps)   mode=--headed; args="$NOHTTPS_ARGS"; preset=1 ;;
  esac
  kill_all
  ...
  printf '{"vm":"%s","arm":"%s","round":%d,"launchUs":%s,"shotStartUs":%s,"shotMs":%d,"rc":%d,"headed":"%s","fixOnCmdline":"%s","preset":"%s","browserDisable":"%s","gpuDisable":"%s","rendererDisable":"%s","rendererEnableHasSync":"%s","page":%s}\n' \
    ...
    "$(case "$rcl" in *InitialWebUISurfaceSync*) printf yes ;; *) printf no ;; esac)" "$v"
...
arms=(headed-nofix headed-fix headless-nofix headless-fix)
for (( r=1; r<=ROUNDS; r++ )); do
  # rotate the order each round
  for (( k=0; k<4; k++ )); do trial "${arms[$(( (k + r) % 4 ))]}" "$r"; done
  if (( r <= 2 )); then trial prod "$r"; trial prod-nohttps "$r"; fi
done
